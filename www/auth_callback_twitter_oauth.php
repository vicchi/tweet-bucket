<?php
	...
	if ($user_id = $twitter_user['user_id']){
		...
		# Even if we do then check to make sure that we've stored their
		# OAuth credentials
		
		if ((!$twitter_user['oauth_token']) ||
				($twitter_user['oauth_token'] != $keys['user_id'])) {
			$update = array (
				'oauth_token' => $data['oauth_token'],
				'oauth_secret' => $data['oauth_token_secret']
			);
			
			$rsp = twitter_users_update_user ($twitter_user, $update);
			
			if (!$rsp['ok']) {
				$GLOBALS['error']['dberr_twitteruser_update'] = 1;
				$GLOBALS['smarty']->display ('page_auth_callback_twitter_oauth.txt');
				exit ();
			}
		}
